...
class Advertise extends AppModel {
    ...
    public function processUpload($check=array()) {
        ...
        if (!empty($check['source']['tmp_name'])) {
            ...
            // build full filename
            $filename = $images_move_dir . Inflector::slug(pathinfo($check['source']['name'], PATHINFO_FILENAME)).'.'.pathinfo($check['source']['name'], PATHINFO_EXTENSION);

            // @todo check for duplicate filename

            // try moving file
            if (!move_uploaded_file($check['source']['tmp_name'], $filename)) {
                return FALSE;
